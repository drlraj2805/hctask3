...
resource "tls_private_key" "t3_key"  {
    algorithm = "RSA"
    rsa_bits =   4096
}
...
resource "local_file" "private_key" {
  depends_on = [
    tls_private_key.t3_key,
  ]
  content = tls_private_key.t3_key.private_key_pem
...
resource "aws_key_pair" "t3_key"{
  key_name = "t3key"
  public_key = tls_private_key.t3_key.public_key_openssh
}
